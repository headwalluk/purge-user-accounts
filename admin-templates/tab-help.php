...
$hwpua_help_sections = array(
	array(
		...
		'steps'   => array(
			...
			__( 'Download the CSV. It is required before accounts are deleted, it is removed from the server after six hours, and the plugin keeps no copy - keep your own.', 'purge-user-accounts' ),
...
$hwpua_help_notes = array(
	...
	__( 'What cannot be undone', 'purge-user-accounts' ) => __( 'Deleting an account and scrambling its password are permanent. Blocking sign-in and stripping roles can both be reversed from the same result.', 'purge-user-accounts' ),
	__( 'Administrators and yourself', 'purge-user-accounts' ) => __( 'Your own account is never acted on, and neither is the last administrator. Other privileged accounts are skipped unless you opt in, and these checks are repeated when an action runs, not only when the query is built.', 'purge-user-accounts' ),
);
